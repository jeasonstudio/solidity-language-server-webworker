...
function wrapper(soljson) {
  const { coreBindings, compileBindings, methodFlags } = (0,
  bindings_1.default)(soljson);
  return {
...
function loadRemoteVersion(versionString, callback) {
  ...
  //       callback(
  //         new Error(`Error retrieving binary: ${response.statusMessage}`)
  //       );
  //     } else {
  ...
  //             module.parent.children.indexOf(soljson),
  //             1
  //           );
  //         }
...
function compileStandardWrapper(compile, inputRaw, readCallback) {
  ...
  try {
    ...
    return (0, formatters_1.formatFatalError)(
      `Invalid JSON supplied: ${e.message}`
    );
  }
  if (input.language !== 'Solidity') {
    return (0, formatters_1.formatFatalError)(
      'Only "Solidity" is supported as a language.'
    );
  }
  ...
  return (0, formatters_1.formatFatalError)(
    'Compiler does not support any known interface.'
  );
}
...
function librariesSupplied(input) {
  if (!(0, helpers_1.isNil)(input.settings)) return input.settings.libraries;
}
function translateOutput(outputRaw, libraries) {
  ...
  try {
    ...
    return (0, formatters_1.formatFatalError)(
      `Compiler returned invalid JSON: ${e.message}`
    );
  }
  const output = translate_1.default.translateJsonCompilerOutput(
    parsedOutput,
    libraries
  );
